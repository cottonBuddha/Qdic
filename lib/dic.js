...
function Dic() {
    this.lookUp = function(word,dic,toSave,read) {
        console.log();
        var wordContent = "";
        for (var key in word) {
        ...
        if (dic === 'google') {
            lookUpGoogle(wordContent,toSave,read)
        }

        if (dic === 'baidu') {
            lookUpInBaidu(wordContent,toSave,read);
        }

        if (dic === 'youdao') {
            lookUpInYouDao(wordContent,toSave,read);
        }
    }

    var lookUpGoogle = function(word,toSave,read) {
        gtoken.get(word).then(function (token) {
            ...
            request(urlStr, function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    output.outputGoogleWordCard(jsonBody,read);
                    if (toSave){
    ...
    var lookUpInBaidu = function(word,toSave,read) {
        var qStr = '?q='+word;
        ...
        request(urlStr, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                var jsonBody = JSON.parse(body);
                output.outputBaiduWordCard(jsonBody,read);
    ...
    var lookUpInYouDao = function(word,toSave,read) {
        ...
        request(urlStr, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                var jsonBody = JSON.parse(body);
                output.outputYoudaoWordCard(jsonBody,read);
